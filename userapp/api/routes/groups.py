...
from userapp.core.schemas.groups import Group as GroupSchema, GroupUpdate, GroupCreate
...
@router.get("")
async def get_groups(response: Response, page: int = 0, page_size: int = 100, filter_query_params=Depends(get_filter_query_params), session=Depends(session_generator)) -> list[GroupSchema]:
...
@router.get("/{group_id}")
async def get_group(group_id: int, session=Depends(session_generator)) -> GroupSchema:
    return await get_one_endpoint(session, GroupTable, group_id)


@router.post("", status_code=201)
async def create_group(group: GroupCreate, session=Depends(session_generator)) -> GroupSchema:
    return await create_one_endpoint(session, GroupTable, group)


@router.put("/{group_id}", status_code=200)
async def update_group(group_id: int, group: GroupUpdate, session=Depends(session_generator)) -> GroupSchema:
    return await update_one_endpoint(session, GroupTable, group_id, group)


@router.get("/{group_id}/users")
async def get_group_users(group_id: int, response: Response, page: int = 0, page_size: int = 100, filter_query_params=Depends(get_filter_query_params), session=Depends(session_generator)) -> List[UserSchema]:
